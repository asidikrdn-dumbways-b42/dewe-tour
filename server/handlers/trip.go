...
func (h *handlerTrip) AddTrip(w http.ResponseWriter, r *http.Request) {
	...
	requestData.TotalQuota, _ = strconv.Atoi(r.FormValue("quota"))
	requestData.Description = r.FormValue("description")

	// mengambil array image url dari context yang dikirim oleh middleware, lalu append ke request Image
	...
	newTrip := models.Trip{
		...
		TotalQuota:     requestData.TotalQuota,
		QuotaAvailable: requestData.TotalQuota,
...
func (h *handlerTrip) UpdateTrip(w http.ResponseWriter, r *http.Request) {
	...
	if r.FormValue("quota") != "" {
		if requestData.TotalQuota >= tripWantToUpdate.QuotaAvailable {
			tripWantToUpdate.TotalQuota = requestData.TotalQuota
		}
	}
...
func convertMultipleTripResponse(t []models.Trip, r *http.Request) []dto.TripResponse {
	...
	for _, trip := range t {
		...
		result.TotalQuota = trip.TotalQuota
		result.QuotaAvailable = trip.QuotaAvailable
...
func convertOneTripResponse(t models.Trip, r *http.Request) dto.TripResponse {
	...
	result.TotalQuota = t.TotalQuota
	result.QuotaAvailable = t.QuotaAvailable
